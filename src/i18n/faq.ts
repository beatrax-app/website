...
export const faqNl: Record<string, { q: string; a: string }> = {
  ...
  "How does sync work without a server?": { q: "Hoe werkt sync zonder server?", a: "Je apparaten praten rechtstreeks met elkaar. Elk apparaat heeft een volledige versleutelde kopie en ze stemmen af via een ondertekend logboek van wijzigingen. Dat gebeurt alleen wanneer ze allebei op hetzelfde netwerk zitten; slaapt er één, dan blijven de wijzigingen staan op het apparaat dat ze maakte tot ze elkaar weer treffen. Een relay is optioneel en dient alleen om de koppeling af te ronden en sleutels uit te wisselen — een transactie gaat er nooit doorheen." },
  "Can you read my data when it syncs?": { q: "Kunnen jullie mijn gegevens lezen als ze synchroniseren?", a: "Nee. Sync is end-to-end versleuteld tussen apparaten die je zelf gekoppeld en geverifieerd hebt, en er is geen Beatrax-account of server die je administratie ooit ziet. Je transacties gaan ook nooit door een relay. Wat daar wél langskomt is de sleuteloverdracht bij het koppelen, verzegeld voor het ontvangende apparaat, en de koppelberichten zelf, die niet versleuteld zijn: apparaat-id's, publieke sleutels en de naam die je een apparaat gaf." },
